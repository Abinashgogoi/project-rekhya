...
export function DashboardApp({ allowDevelopmentShell }: { allowDevelopmentShell: boolean }) {
  ...
  if (configured && session && !profile) return <main className="auth-page"><section className="card auth-card"><h1>Project Rekhya</h1><p>Preparing your officer profile…</p></section></main>;
...
function OperationalDashboard({ session, profile, developmentShell }: { session: Session | null; profile: Profile | null; developmentShell: boolean }) {
  ...
  async function loadEvidencePreview(item: EvidenceItem) {
    if (!session || item.previewUrl) return;
    setEvidencePreviewBusy(item.id); setError(null);
    try {
      const response = await fetch(`/api/evidence/${item.id}`, { headers: { Authorization: `Bearer ${session.access_token}` } });
      if (!response.ok) {
      ...
      setEvidenceItems((current) => current.map((entry) => entry.id === item.id ? { ...entry, previewUrl: url } : entry));
    } catch (reason) {
  ...
  async function issueAgentCommand(command: "start" | "pause" | "resume" | "retry_pending" | "stop_safely" | "restart" | "single") {
    ...
    if (command === "start") response = await supabase.rpc("queue_verification_run", { p_start: appliedRange.start, p_end: appliedRange.end });
      ...
      if (!userId) { setError("Enter the exact User ID for the single-ID run."); return; }
