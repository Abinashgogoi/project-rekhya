...
function OperationalDashboard({ session, profile, developmentShell }: { session: Session | null; profile: Profile | null; developmentShell: boolean }) {
  ...
  async function prepareAndroid() {
    ...
    // Launch the installed bridge directly from the trusted browser click.
    // Chromium may block a custom protocol after an awaited network request.
  ...
  async function issueAgentCommand(command: "start" | "pause" | "resume" | "retry_pending" | "stop_safely" | "single") {
    if (!supabase || !session || !canUseTechnical) return;
    setError(null);
    if (!liveStatusEnabled) { setError("Turn Technical Live Status ON before issuing Android-agent controls."); return; }
    if (command === "start" && !window.confirm("Start a fresh full verification run from the first User ID? Any unfinished previous run will not be resumed.")) return;
    let response: { error: { message: string } | null };
    // Start Full Run owns fresh-start semantics. Resume latest is the only
    // control that continues an unfinished run/checkpoint.
    if (command === "start") response = await supabase.rpc("restart_verification_from_beginning", { p_start: appliedRange.start, p_end: appliedRange.end });
