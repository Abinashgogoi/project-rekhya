...
export async function POST(request: Request) {
  try {
    ...
    const records = await Promise.all(body.records.map(async (record) => ({ name: record.name, user_id: record.userId, password_ciphertext: await encryptCredential(record.password), password_missing: !record.password, block: record.block, group: record.group, source_row_number: record.sourceRowNumber, extra_fields: record.rawFields })));
